...
def get_quiz_attempts(user_id):
    return (
        db.session.query(
            ...
            Score_Table.attempt_id,  # Use Score_Table's attempt_id since it represents a full quiz attempt
...
@user_bp.route('/progress/<username>')
def quiz(username):
...
@user_bp.route('/quiz/attempt/<int:quiz_id>/<username>', methods=['GET', 'POST'])
...
@user_bp.route('/quiz/<int:quiz_id>/view/<username>/<int:attempt_id>')
...
@user_bp.route("/todays-quizzes/<username>")
